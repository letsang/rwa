...
Environment.prototype._buildHDRISky = function () { this.scene.environmentIntensity = 0.62; };
...
assert.equal(uniforms.sunIntensity, 0.71);
assert.ok(Math.abs(uniforms.ambIntensity - (0.64 / 1.08)) < 0.0001);
...
assert.equal(environment.pipeline, null, 'CURRENT supprime la passe Sharpen/Grain devenue inutile');
