...
# ============================================================
# 0. SELECTION DU STATIC MESH MESHY
# ============================================================

selected = unreal.EditorUtilityLibrary.get_selected_assets()

if not selected:
    raise RuntimeError("Sélectionne le Static Mesh Meshy.")
...
manny = unreal.EditorAssetLibrary.load_asset(
    "/Game/Characters/Mannequins/Meshes/SKM_Manny_Simple"
)

if not manny:
    raise RuntimeError("SKM_Manny_Simple introuvable.")
...
manny_min_z = (
    manny_bounds.origin.z -
    manny_bounds.box_extent.z
)
...
# Scale uniforme automatique

scale = manny_height / meshy_height


# Centre du Meshy avant scale
...
# Translation finale pour :
# - centrer X/Y sur Manny
# - mettre les pieds au même Z que Manny

tx = (
    manny_bounds.origin.x -
    meshy_center_x * scale
)

ty = (
    manny_bounds.origin.y -
    meshy_center_y * scale
)

tz = (
    manny_min_z -
    meshy_bounds.min.z * scale
)
...
_, root_name = (
    unreal.GeometryScript_BoneWeights.get_root_bone_name(
        meshy_dm
    )
)
...
# ============================================================
# 7. V2 SAFE - TRANSFERT DES SKIN WEIGHTS
# ============================================================
...
# Méthode validée comme fonctionnelle
...
try:
    ...
    unreal.log_warning(
        f"output_target_mesh_bones non modifié : {e}"
    )


# Pas d'Inpaint / smoothing ici
...
_, has_weights = (
    unreal.GeometryScript_BoneWeights.mesh_has_bone_weights(
        meshy_dm
    )
)
...
unreal.log("")
unreal.log(
    f"Bone weights V2 SAFE présents : {has_weights}"
)


if not has_weights:
    raise RuntimeError(
        "Le transfert des bone weights a échoué."
    )
...
for vertex_id in range(vertex_count):
    ...
    # --------------------------------------------------------
    # Supprimer les micro-influences
    # --------------------------------------------------------

    for bw in weights:

        bone_index = bw.get_editor_property(
            "bone_index"
        )

        weight = bw.get_editor_property(
            "weight"
        )
    ...
    # --------------------------------------------------------
    # Sécurité si tout a été supprimé
    # --------------------------------------------------------
    ...
    if not cleaned:
        ...
        strongest = max(
            weights,
            key=lambda x:
                x.get_editor_property("weight")
        )

        cleaned = [
            (
                strongest.get_editor_property(
                    "bone_index"
                ),
                1.0
            )
    ...
    # --------------------------------------------------------
    # Garder uniquement les 4 influences principales
    # --------------------------------------------------------
...
unreal.log("===== V3 CLEAN =====")
unreal.log(
    f"Vertices nettoyés     : {cleaned_vertices}"
)
unreal.log(
    f"Micro-poids supprimés : {removed_weights}"
)
unreal.log(
    f"Seuil                 : {MIN_WEIGHT}"
)
unreal.log(
    f"Max influences cible  : {MAX_INFLUENCES}"
)
unreal.log("====================")


# ============================================================
# 9. CHEMIN DU NOUVEL ASSET
# ============================================================
...
folder, source_name = (
    source_package.rsplit("/", 1)
)


# Nettoyer les suffixes issus de nos tests précédents
...
# Retirer éventuellement un ancien prefix SKM_
...
height_ok = (
    abs(height - manny_height) < 3.0
)

ground_ok = (
    abs(min_z - manny_min_z) < 1.0
)
...
for vertex_id in range(vertex_count):
    ...
    active_weights = [
        ...
        bw for bw in weights

        if (
            bw.get_editor_property("weight")
            > WEIGHT_EPSILON
        )
    ...
    max_influences_found = max(
        max_influences_found,
        influence_count
    )
...
unreal.log(
    f"IK/control weights ... "
    f"{bad_non_deform_weights}   "
...
ready = (
    ...
    and
    max_influences_found <= MAX_INFLUENCES
...
# ============================================================
# 12. CREER LE SKELETAL MESH FINAL
# ============================================================
...
# Conserver les matériaux si l'option est exposée
...
unreal.log("")
unreal.log(
    f"Create Skeletal Mesh : {outcome}"
)
...
if not new_skm:
    ...
    raise RuntimeError(
        "Création du Skeletal Mesh impossible."
    )
...
if not ready:
    unreal.log_warning(
        "GLB NON EXPORTE : le Rig Quality Check n'est pas READY."
    )
else:
    ...
    unreal.log(f"Source : {new_skm.get_name()}")
    unreal.log(f"Output : {output_file}")

    gltf_options = unreal.GLTFExportOptions()

    gltf_options.set_editor_property(
    ...
    # Les animations Manny restent dans Unreal.
    gltf_options.set_editor_property(
    ...
    glb_success = unreal.GLTFExporter.export_to_gltf(
        new_skm,
        output_file,
        gltf_options,
        set()
...
# ============================================================
# 15. PIPELINE FINAL RESULT
# ============================================================

unreal.log("")
unreal.log("============================================")
unreal.log("===== RWA CHARACTER PIPELINE ===============")
unreal.log("============================================")
unreal.log(f"Input    : {meshy_static.get_name()}")
unreal.log(f"SKM      : {new_path}")
unreal.log(f"Rig QC   : {'READY' if ready else 'REVIEW'}")

if ready:
    unreal.log(f"GLB      : {'SUCCESS' if glb_success else 'FAILED'}")

    if glb_success:
        unreal.log(f"File     : {output_file}")
        unreal.log("")
        unreal.log(">>> CHARACTER READY FOR CLAUDE <<<")
else:
    unreal.log("GLB      : SKIPPED")
