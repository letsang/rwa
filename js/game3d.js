...
class Game3D {
  ...
  buildWorld() {
    ...
    // anneau de sélection partagé
    const sel = BABYLON.MeshBuilder.CreateTorus('selRing', { diameter: 3.2, thickness: 0.18, tessellation: 32 }, scene);
    sel.material = this.mat('sel', '#ffffff', { emissive: '#ffffff' });
    sel.isPickable = false; sel.setEnabled(false);
    this.selRing = sel;
    // (cercle jaune de ciblage aimRing retiré — inutile)
    this.aimRing = null;
  }
  ...
  updateMeshes(dt) {
    ...
    // sélection
    const tg = p.target;
    if (tg && !tg.dead && this.isEnemyVisible(tg)) { this.selRing.setEnabled(true); this.selRing.position.set(tg.x * S, this.getTerrainHeight(tg.x, tg.y) + 0.1, tg.y * S); }
    else this.selRing.setEnabled(false);
    // (aimRing retiré)
    // hit fx
